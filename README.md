...
Note:
  * You should **NOT** replace your `cookies.txt` with `cookies.txt` witch generated by `live-dl` and ~~should increase the `interval` to at least **5 minutes**~~ You can use `LONG INTERVAL` below to avoid YouTube seeing your account as a bot.
...
* Open https://discord.com/channels/@me/ on browser, select your Bot and copy channel ID (numbers after /@me/, put **between two "**) to `config.yml` file
